...
<?php	
		...
		echo'<thead><tr><th>Your Files</th> <th>&nbsp;</th> <th>Date</th> <th>Category</th> <th>&nbsp;</th></tr> </thead>';
	...
	"SELECT $wpdb->users.ID FROM $wpdb->users ORDER BY %s ASC", $szSort ));
			...
			$user_info = get_userdata( $iUserID );
			   ...
			   echo '<tr><td>'.$user_info->user_login;
				...
				echo '<td><form method="get" action="admin.php?page=files-see-ftp"><input type="hidden" name="createFolder" value="'.$iUserID.'" /><input type="submit" value="';echo __('Create Folder','userfiles');
                echo'" /></form></td></tr>';				
